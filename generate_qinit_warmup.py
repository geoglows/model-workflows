...
return_period_ncs = glob.glob('/Users/ricky/Downloads/rps/*.nc')
...
def main():
    # Memory shouldn't be an issue since return period files are so small
    processes = multiprocessing.cpu_count()
    with multiprocessing.Pool(processes=processes) as pool:
        pool.map(helper, [sublist for sublist in list_to_sublists(return_period_ncs,processes)])
    print('Finished')
...
def list_to_sublists(lst: list, n: int) -> list[list[str]]:
